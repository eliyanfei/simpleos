 ...
 * 2013-12-3下午12:29:55
 */
public class DataTempAppModule extends AItSiteAppclicationModule implements IDataTempAppModule {

	public static Table catalog = new Table("simpleos_menu_catalog", "id");

	@Override
	protected void putTables(final Map<Class<?>, Table> tables) {
		tables.put(ProductCatalogBean.class, catalog);
	}
	...
	public void init(final IInitializer initializer) {
		...
		doInit(DataTempUtils.class, deployName);
		DataTempUtils.appModule = this;
		try {
			...
			try {
				...
				for (final String subType : subTypes) {
					...
					if (null == impl)
						continue;
					if (impl.isMenu()) {
						if (count("name=?", new Object[] { impl.getName() }, ProductCatalogBean.class) == 0) {
							ProductCatalogBean menuBean = new ProductCatalogBean();
