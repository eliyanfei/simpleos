 ...
 * 2013-12-3下午01:42:25
 */
public class ProductItemHandle extends DefaultCatalogHandle {
	@Override
	public Class<? extends IIdBeanAware> getEntityBeanClass() {
		return ProductCatalogBean.class;
	}

	@Override
	public IApplicationModule getApplicationModule() {
		return DataTempUtils.appModule;
	}
	...
	public Collection<? extends AbstractTreeNode> getCatalogTreenodes(final ComponentParameter compParameter, final AbstractTreeBean treeBean,
		...
		for (final AbstractTreeNode treeNode2 : coll) {
			if (treeNode == null) {
				...
				final ProductCatalogBean sysDict = (ProductCatalogBean) treeNode2.getDataObject();
				if (sysDict != null) {
				}
			}
			if (treeNode == null && !dictionary) {
				treeNode2.setImage(DictUtils.getCssPath(compParameter) + "/images/dict.png");
				final ProductCatalogBean sysDict = getEntityBeanById(compParameter, getDocumentId(compParameter));
				if (sysDict != null) {
	...
	public Collection<PropField> getPropFields(final ComponentParameter compParameter, final PropEditorBean formEditor) {
		return DataTempUtils.appModule.doDictItemPropEditor(compParameter.request, compParameter.response, (CatalogBean) compParameter.componentBean,
				formEditor);
	}
