 ...
 * 2013-12-3下午01:42:25
 */
public class MenuNavItemHandle extends DefaultCatalogHandle {
	@Override
	...
	public Collection<? extends AbstractTreeNode> getCatalogTreenodes(final ComponentParameter compParameter, final AbstractTreeBean treeBean,
		...
		for (final AbstractTreeNode treeNode2 : coll) {
			if (treeNode == null) {
				...
				final MenuNavBean sysDict = (MenuNavBean) treeNode2.getDataObject();
				if (sysDict != null) {
				}
			}
			if (treeNode == null && !dictionary) {
				treeNode2.setImage(DictUtils.getCssPath(compParameter) + "/images/dict.png");
				final MenuNavBean sysDict = getEntityBeanById(compParameter, getDocumentId(compParameter));
				if (sysDict != null) {
	...
	public Collection<PropField> getPropFields(final ComponentParameter compParameter, final PropEditorBean formEditor) {
		return MenuNavUtils.appModule.doDictItemPropEditor(compParameter.request, compParameter.response, (CatalogBean) compParameter.componentBean,
				formEditor);
	}
