 ...
 */
public class NewsMgrPagerHandle extends DefaultNewsPagerHandle {
	...
	class AppNewsTablePagerData extends NewsTablePagerData {
		...
		private String getTitle(final NewsBean news) {
			...
			sb.append(ContentUtils.getShortContent(news, 100, true));
			...
			sb.append("<span style=\"margin: 0px 5px;\">|</span>");
			sb.append("<span class=\"num\">" + (ConvertUtils.toDateString(news.getCreateDate(), "yyyy-MM-dd HH:mm")) + "</span>&nbsp;");
			sb.append("</div>");
			sb.append("</div>");
			sb.append("</div>");
			return sb.toString();
