 ...
 * 2014-1-5下午02:09:01
 */
public class UserLastRegisterPortalModule extends AbstractContentLayoutModuleHandle {
	public UserLastRegisterPortalModule(final PageletBean pagelet) {
		super(pagelet);
	}

	private static String[] defaultOptions = new String[] { "_user_r_rows=6", "_user_r_size=64" };
	...
	public IForward getPageletOptionContent(final ComponentParameter compParameter) {
		return new UrlForward(UserRegisterUtils.getHomePath() + "/jsp/user_last_register_option.jsp");
	}
	...
	public IForward getPageletContent(final ComponentParameter compParameter) {
		compParameter.setRequestAttribute("__qs",
				OrgUtils.am().query(new ExpressionValue("status=? order by createdate desc", new Object[] { EAccountStatus.normal })));
		String forward = OrgUtils.deployPath + "jsp/account_layout.jsp?";
		forward = WebUtils.addParameters(forward, "showCreateDate=true");
		forward = WebUtils.addParameters(forward, "size=" + ConvertUtils.toInt(getPagelet().getOptionProperty("_user_r_size"), 64));
		forward = WebUtils.addParameters(forward, "rows=" + ConvertUtils.toInt(getPagelet().getOptionProperty("_user_r_rows"), 6));
