...
public abstract class UserRegisterUtils {
	...
	public static void sentMailActivation(final IAccount account,
			final Class<?> c, final String filename) {
		...
		final String url = serverUrl
				+ getHomePath()
				+ "/jsp/regist_activation.jsp?code="
				+ StringUtils.hash(ConvertUtils.toDateString(account
						.getCreateDate())) + "&accountId=" + account.getId();
		...
		try {
			mailMessage.setTextBody(ScriptEvalUtils
					.replaceExpr(variable,
							IoUtils.getStringFromInputStream(c.getClassLoader()
									.getResourceAsStream(
											BeanUtils.getResourceClasspath(c,
													filename)))));
	...
	public static String getHomePath() {
		return AbstractComponentRegistry.getRegistry(
				UserRegisterRegistry.userRegister).getResourceHomePath();
	}
}
