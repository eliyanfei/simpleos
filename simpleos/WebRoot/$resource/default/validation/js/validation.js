...
Validation.insertAfter = function(ele, message, d) {
	ele = $(ele);
	if (!ele)
		return;
	if (!ele.insertAfter) {
		d = d || "div";
		ele.insertAfter = new Element(d, {
...
Validation.methods = {
	...
	'length_range' : function(v, args) {
		return Validation.isEmpty(v)
				|| (args.length > 1
						&& (args[2] = v.length) >= parseInt(args[0]) && args[2] <= parseInt(args[1]));
	},
...
Validation.prototype = {
	...
	validate : function() {
		return !this.validators.find((function(validator) {
			...
			return $$(validator.selector).find((function(ele) {
				...
				if (!this.getMethod(method)(fv, args)) {
						...
						if (wt == 'alert') {
							...
							Validation.insertAfter(ele, message,this.options.d);
