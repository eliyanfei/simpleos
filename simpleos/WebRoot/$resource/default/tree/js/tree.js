var TafelTree = Class.create();TafelTree.scriptFragment = /[\s]*<[/]?[ul|li].*>.*/ig;
...
TafelTree.loadFromUL = function (id, imgBase, width, height, options) {
	...
	var load = new Element('img', {
		'title' : 'load',
		'alt'   : 'load',
		'src'   : ((imgBase) ? imgBase : 'imgs/') + 'load.gif'
	});
	...
	for (var i = 0; i < obj.childNodes.length; i++) {
		if (obj.childNodes[i].nodeName.toLowerCase() == 'li') {
			str += this._loadFromUL(obj.childNodes[i], virgule);
			virgule = ',';
		}
	}
	str += '])';
...
TafelTree._loadFromUL = function (obj, virgule) {
	...
	TafelTree.textAttributes.each(function (attr) {
		if (obj.getAttribute(attr)) str += "," + "'" + attr.replace('T', '') + "' : '" + obj.getAttribute(attr) + "'";
	});
		...
		if (obj.childNodes[i].nodeName.toLowerCase() == 'ul') {
			...
			for (var j = 0; j < obj.childNodes[i].childNodes.length; j++) {
				if (obj.childNodes[i].childNodes[j].nodeName.toLowerCase() == 'li') {
					str += this._loadFromUL(obj.childNodes[i].childNodes[j], virgule);
					virgule = ',';
				}
			}
			str += ']';
...
TafelTree.prototype = {
	...
	setOptions : function (options) {
		...
		var so = function(key, m) {
			if (typeof(options[key]) != 'undefined' || options[key]) m(options[key]);
		};
		so("dropALT", this.disableDropALT.bind(this));
		so("dropCTRL", this.disableDropCTRL.bind(this));
		...
		so("openAfterAdd", this.openAfterAdd.bind(this));		
		so("showSelectedBranch", this.showSelectedBranch.bind(this));
		so("reopenFromServer", this.reopenFromServerAfterLoad.bind(this));
...
TafelTreeBaseBranch.prototype = {
	...
	_getimg : function(img) {
		if (img.charAt(0) == "/") {
			if (!img.startsWith(CONTEXT_PATH)) {
	...
	select : function(ev) {
		if(this.tree.lastclassSelected){
			Element.removeClassName(this.tree.lasttxt, this.tree.lastclassSelected);
		}
		...
		this.tree.lasttxt = this.txt;
		this.tree.lastclassSelected =this.tree.classSelected;
	...
	_setProperties : function() {
		...
		if (typeof (this.struct.check) == 'undefined' || (this.tree.useCookie && this.tree.cookieChecked))
			this.struct.check = 0;

		if (typeof (this.struct.items) == 'undefined') {
			this.struct.items = [];
		}
		if (typeof (this.struct.canhavechildren) == 'undefined')
			this.struct.canhavechildren = false;
		if (typeof (this.struct.id) == 'undefined')
			this.struct.id = this.idObj;
		if (typeof (this.struct.acceptdrop) == 'undefined')
			this.struct.acceptdrop = true;
		if (typeof (this.struct.last) == 'undefined')
			this.struct.last = false;
		if (typeof (this.struct.editable) == 'undefined')
			this.struct.editable = this.tree.editableBranches;
		if (typeof (this.struct.checkbox) == 'undefined')
			this.struct.checkbox = true;
	},
	...
	_setFunctions : function() {
		if (typeof (this.struct.onerrorajax) == 'undefined') {
			...
			this.struct.onerrorajax = eval(this.struct.onerrorajax);
		}
		if (typeof (this.struct.oneditajax) == 'undefined') {
			...
			this.struct.oneditajax = eval(this.struct.oneditajax);
		}
		if (typeof (this.struct.onopenpopulate) == 'undefined') {
			...
			this.struct.onopenpopulate = eval(this.struct.onopenpopulate);
		}
		if (typeof (this.struct.onedit) == 'undefined') {
			...
			this.struct.onedit = eval(this.struct.onedit);
		}
		if (typeof (this.struct.oncheck) == 'undefined') {
			...
			this.struct.oncheck = eval(this.struct.oncheck);
		}
		if (typeof (this.struct.onbeforecheck) == 'undefined') {
			...
			this.struct.onbeforecheck = eval(this.struct.onbeforecheck);
		}
		if (typeof (this.struct.onopen) == 'undefined') {
			...
			this.struct.onopen = eval(this.struct.onopen);
		}
		if (typeof (this.struct.onbeforeopen) == 'undefined') {
			...
			this.struct.onbeforeopen = eval(this.struct.onbeforeopen);
		}
		if (typeof (this.struct.onmouseover) == 'undefined') {
			...
			this.struct.onmouseover = eval(this.struct.onmouseover);
		}
		if (typeof (this.struct.onmouseout) == 'undefined') {
			...
			this.struct.onmouseout = eval(this.struct.onmouseout);
		}
		if (typeof (this.struct.onmousedown) == 'undefined') {
			...
			this.struct.onmousedown = eval(this.struct.onmousedown);
		}
		if (typeof (this.struct.onmouseup) == 'undefined') {
			...
			this.struct.onmouseup = eval(this.struct.onmouseup);
		}
		if (typeof (this.struct.onclick) == 'undefined') {
			...
			this.struct.onclick = eval(this.struct.onclick);
		}
		if (typeof (this.struct.ondblclick) == 'undefined') {
			...
			this.struct.ondblclick = eval(this.struct.ondblclick);
		}
	},
	...
	_setContextMenu : function() {
		if (this.struct.contextMenu) {
			var o = this.txt;
			$Actions.callSafely(this.struct.contextMenu, null, function(act) {
				act.menu.setOptions( {
...
TafelTreeBranch.prototype = Object.extend(new TafelTreeBaseBranch, {
	initialize : function(root, parent, struct, level, before, after, pos) {
		...
		if (typeof (this.struct.draggable) == 'undefined')
			this.struct.draggable = 1;
		this._setProperties();
		...
		if (typeof (this.struct.ondrop) == 'undefined') {
			...
			this.struct.ondrop = eval(this.struct.ondrop);
		}
		if (typeof (this.struct.ondropajax) == 'undefined') {
			...
			this.struct.ondropajax = eval(this.struct.ondropajax);
		}
		this._setFunctions();
...
function __tree_sibling(branch, up) {
	return up ? branch.getPreviousSibling() : branch.getNextSibling();
}

function __tree_move(branch, up) {
	if (!branch)
		return;
	var sibling = __tree_sibling(branch, up);
	if (!sibling)
		return;
	return "up=" + up + "&b1=" + branch.getId() + "&b2=" + sibling.getId();
}

function __tree_move2(branch, up) {
	if (!branch)
		return;
	var sibling = __tree_sibling(branch, up);
	var sibling2;
	while (sibling && (sibling2 = __tree_sibling(sibling, up))) {
		sibling = sibling2;
	}
	if (!sibling)
		return;
...
function __tree_getSelects(tree, branch, ev) {
	var selects = [];
  ...
  if (branch) {
     if (branch.checkbox) branch.check(true);
     else selects.push(create(branch));
  }
  tree.getCheckedBranches().each(function(b) { selects.push(create(b)); });
  return selects;
}	
...
function __tree_actions_init(actionFunc) {
	...
	var __click = function(branch, ev) {
		var cb = function(id, text, branch, ev) {
			if (branch.struct.jsClickCallback) {
				eval(branch.struct.jsClickCallback);
			} else {
				var t = branch.getTree();
				if (t.jsClickCallback) {
					eval(t.jsClickCallback);
				}
			}
		};
		cb(branch.getId(), branch.getText(), branch, ev);
	};
	
	var __dblclick = function(branch, ev) {
		var cb = function(id, text, branch, ev) {
			if (branch.struct.jsDblclickCallback) {
				eval(branch.struct.jsDblclickCallback);
			} else {
				var t = branch.getTree();
				if (t.jsDblclickCallback) {
					eval(t.jsDblclickCallback);
				}
			}
		};
		cb(branch.getId(), branch.getText(), branch, ev);
	...
	var __check = function(branch, checked, ev) {
		var cb = function(id, text, branch, checked, ev) {
			if (branch.struct.jsCheckCallback) {
				eval(branch.struct.jsCheckCallback);
			} else {
				var t = branch.getTree();
				if (t.jsCheckCallback) {
					eval(t.jsCheckCallback);
				}
			}
		};
		cb(branch.getId(), branch.getText(), branch, checked, ev);
	};
	...
	var __dropajax = function(branchMoved, newParent, response, dropFinished) {
		if (!dropFinished) {
			...
			if (json['responseText']) {
				new $Comp.AjaxRequest(null, json['responseText'], json['ajaxRequestId']);
			}
			return json['dropOk'];
		}
		return true;
	...
	actionFunc.refresh = function(params) {
		...
		new Ajax.Request(actionFunc.treeUrl("ajax_refresh.jsp", params), {
			onComplete: function(req) {
				try {
					actionFunc.createTree(eval(req.responseText));
				} catch(e) {
					$Loading.hide(); 
