...
public class PrjLinksUtils {
	...
	public static String queryLinks(final PageRequestResponse requestResponse,
		...
		if ("custom".equals(map.get("links_linksType"))) {
			...
			IQueryEntitySet<PrjLinksBean> qs = appModule.queryBean(
					new ExpressionValue("1=1 order by oorder"),
					PrjLinksBean.class);
			...
			while ((linksBean = qs.next()) != null) {
				b1.append(
						"<a style='color:" + linksBean.getColor() + ";' href='"
								+ linksBean.getUrl() + "' target='blank'>")
						.append(linksBean.getTitle()).append("</a>");
			}
		}
		b1.append("<a  href='/links.html' target='blank'>").append("更多链接")
				.append("</a>");
		return b1.toString();
	}
