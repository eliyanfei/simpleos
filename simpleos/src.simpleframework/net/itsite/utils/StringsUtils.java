...
public final class StringsUtils {
	 ...
	 * @return 加密后的字符串
	 */
	public synchronized static final String md5(final String data) {
		try {
	...
	public static String replaceParam(final String paramName, final String src, final String newParamValue) {
		...
		if (null == paramName || "".equals(paramName))
			return src;
		// 如果传入的参数中指定的原来就不存在值,直接返回新参数的名称和值.
		if (null == src || "".equals(src))
			return paramName + newParamValue + ";";
		...
		while (st.hasMoreElements()) {
			...
			if (!"".equals(TMP)) {
				if (TMP.startsWith(paramName)) {
					// 如果已经标志为改变过此参数.说明有重复的参数.跳过继续执行下一个.
					if (modif) {
						continue;
					}
	...
	public static final boolean likesContains(final String[] array, final String valueItem) {
		if (null == array || isBlank(valueItem))
			return false;
	...
	public static final boolean likesOut(final String[] array, final String valueItem) {
		...
		for (final String value : array) {
			if (value.equals(valueItem) || value.startsWith(valueItem) || value.endsWith(valueItem))
				return true;
		}
		return false;
	}
	...
	public static String toParam_Value(final String paramName, final String paramValue) {
		if (isNotBlank(paramName) && isNotBlank(paramValue))
			return " " + paramName + "=\"" + paramValue + "\"";
		return "";
	}
	...
	public static final String infoWithDateTime(final String logInfo, final boolean before) {
		String format;
		if (before)
			format = unionString("[LOG] ", logInfo, " %1$tY-%<tm-%<td %<tH:%<tM:%<tS \n");
		else
			format = unionString("[LOG] %1$tY-%<tm-%<td %<tH:%<tM:%<tS \n", logInfo);
	...
	public static final String like(final Collection<String> array, final String dest) {
		if (null == array || array.size() == 0 || isBlank(dest))
			return "__NULL_LIKE__";
		...
		for (final String src : array) {
			...
			if (curLiked == src.length() + 1)// equals
			{
				maxLikedValue = src;
				break;
			}
	...
	public static final <T> List<List<T>> splitList(final List<T> srcList, final int splitIdx) {
		...
		for (int i = 0; i < cnt; i++) {
			...
			if (gidx + splitIdx > size)
				tmps = srcList.subList(gidx, size);
			else
				tmps = srcList.subList(gidx, gidx + splitIdx);
			result.add(tmps);
			gidx += splitIdx;
		}
		return result;
	...
	public static final Map<String, List<String>> dialogformat(String str) {
		...
		// System.out.println(strtable.length);
		for (final String element : strtable) {
			final List<String> temp = new ArrayList<String>();
			String tablename = null;
			String colname = null;
			// System.out.println(strtable[i].toString());
	...
	public static boolean isNotBlank1(final String str) {
