...
import net.simpleframework.organization.component.register.UserLastLoginPortalModule;
import net.simpleframework.organization.component.register.UserLastRegisterPortalModule;
...
public class OrganizationApplicationModule extends AbstractWebApplicationModule implements IOrganizationApplicationModule {
	...
	public void init(final IInitializer initializer) {
		...
		((ITaskExecutorAware) getApplication()).getTaskExecutor().addScheduledTask(DateUtils.HOUR_PERIOD, DateUtils.DAY_PERIOD * 2,
		...
		PortalModuleRegistryFactory.regist(UserLastRegisterPortalModule.class, "user_register", "最新注册用户", "信息",
				"/$resource/default/userRegister/images/user_register.png", "最新注册的用户");
		PortalModuleRegistryFactory.regist(UserLastLoginPortalModule.class, "user_login", "最后登入用户", "信息",
				"/$resource/default/userRegister/images/user_login.png", "最后登入的用户");
	...
	public void mailNoLogin(final IAccount account) {
		...
		try {
			mailMessage.setTextBody(ScriptEvalUtils.replaceExpr(
					variable,
					IoUtils.getStringFromInputStream(OrganizationApplicationModule.class.getClassLoader().getResourceAsStream(
							"net/simpleframework/organization/account_no_login.html"))));
	...
	protected void doAccountMail() {
		...
		while ((account = (IAccount) qs.next()) != null) {
			if (account.getStatus() == EAccountStatus.normal) {
				NotificationLogBean log = log_mgr.queryForObject(new ExpressionValue("toId=? and notificationEvent=? order by sentDate desc",
						new Object[] { account.getId(), ENotificationEvent.mail_no_login }), NotificationLogBean.class);
