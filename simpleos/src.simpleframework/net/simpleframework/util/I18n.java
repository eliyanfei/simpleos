...
public abstract class I18n {
	...
	private static ResourceBundle getResourceBundle(final String basename, final Locale locale) {
		synchronized (cachedResourceBundles) {
			...
			try {
				...
				if (localeMap == null) {
					cachedResourceBundles.put(basename, localeMap = new HashMap<Locale, ResourceBundle>());
				}
	...
	private static final Pattern MESSAGE_PATTERN = Pattern.compile("[\\s\\S]*(\\#\\([\\w\\.]+\\))[\\s\\S]*");
	...
	public static String replaceI18n(String template) {
		...
		while (true) {
			...
			if (matcher.matches()) {
				...
				template = template.substring(0, result.start(1)) + object + template.substring(result.end(1));
			} else {
				break;
			}
		}
