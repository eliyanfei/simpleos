...
public abstract class DateUtils {
	...
	public static String getDifferenceDate(long dur) {
		dur = dur / 1000;
		final String s = LocaleI18n.getMessage("DateUtils.0");
		final String m = LocaleI18n.getMessage("DateUtils.1");
		final String h = LocaleI18n.getMessage("DateUtils.2");
		final String d = LocaleI18n.getMessage("DateUtils.3");
		final String p = "";
	...
	public static String getRelativeDate(final Date date) {
		...
		final StringBuilder sb = new StringBuilder();
		if (days != 0L) {
			sb.append(days).append(LocaleI18n.getMessage("DateUtils.3"));
		} else if ((hours -= days * 24L) != 0L) {
			sb.append(hours).append(LocaleI18n.getMessage("DateUtils.2"));
		} else if ((mins -= (days * 24L + hours) * 60L) != 0L) {
			sb.append(mins).append(LocaleI18n.getMessage("DateUtils.1"));
		}
		if (days != 0L || hours != 0L || mins != 0L) {
			sb.append(LocaleI18n.getMessage("DateUtils.4"));
		} else {
			sb.append(LocaleI18n.getMessage("DateUtils.5"));
		}
		return sb.toString();
	}
	...
	public static enum TimeDistance {
		...
		public static TimeDistance whichOne(String name) {
			final TimeDistance[] types = TimeDistance.values();
			for (TimeDistance type : types) {
