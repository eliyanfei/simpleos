...
public class MyFriendsSelectedHandle extends DefaultUserSelectHandle {
	@Override
	public IDataObjectQuery<? extends IUser> getUsers(final ComponentParameter compParameter,
			final UserPagerBean userPager) {
		...
		sql.append(" t right join ").append(
				FriendsUtils.getTableEntityManager(Friends.class).getTablename());
		sql.append(" a on a.friendid=t.id where a.userid=?");
		final String user_name = compParameter.getRequestParameter(OrgUtils.um()
				.getUserTextParameterName());
		if (StringUtils.hasText(user_name)
				&& !user_name.equals(LocaleI18n.getMessage("user_select.3"))) {
			sql.append(" and (t.name like '%").append(user_name).append("%' or t.text like '%")
